...
void contractor_id::prune(box &, SMTConfig &, std::vector<box> &) {
    return;
}
...
void contractor_seq::prune_naive(box & b, SMTConfig & config, std::vector<box> & bin) {
...
void contractor_seq::prune(box & b, SMTConfig & config, std::vector<box> & bin) {
    if (m_vec.size() == 0) {
        return;
    }
    return prune_naive(b, config, bin);
}
...
void contractor_try::prune(box & b, SMTConfig & config, std::vector<box> & bin) {
    DREAL_LOG_DEBUG << "contractor_try::prune: ";
    thread_local static box old_box(b);
    old_box = b;
    try {
        m_c.prune(b, config, bin);
        m_output = m_c.output();
...
void contractor_try_or::prune(box & b, SMTConfig & config, vector<box> & bin) {
    DREAL_LOG_DEBUG << "contractor_try_or::prune";
    thread_local static box old_box(b);
    old_box = b;
    try {
        m_c1.prune(b, config, bin);
        m_output = m_c1.output();
        m_used_constraints = m_c1.used_constraints();
    } catch (contractor_exception & e) {
        b = old_box;
        m_c2.prune(b, config, bin);
...
void contractor_throw_if_empty::prune(box & b, SMTConfig & config, vector<box> & bin) {
...
void contractor_join::prune(box & b, SMTConfig & config, vector<box> & bin) {
...
void contractor_ite::prune(box & b, SMTConfig & config, vector<box> & bin) {
    DREAL_LOG_DEBUG << "contractor_ite::prune";
    if (m_guard(b)) {
        m_c_then.prune(b, config, bin);
        m_output = m_c_then.output();
        m_used_constraints = m_c_then.used_constraints();
        return;
    } else {
        m_c_else.prune(b, config, bin);
...
void contractor_fixpoint::naive_fixpoint_alg(box & b, SMTConfig & config, vector<box> & bin) {
...
void contractor_fixpoint::worklist_fixpoint_alg(box & b, SMTConfig & config, vector<box> & bin) {
...
void contractor_int::prune(box & b, SMTConfig & config, vector<box> &) {
...
void contractor_eval::prune(box & b, SMTConfig & config, vector<box> &) {
...
void contractor_cache::prune(box & b, SMTConfig & config, vector<box> & bin) {
...
void contractor_sample::prune(box & b, SMTConfig &, vector<box> &) {
...
void contractor_aggressive::prune(box & b, SMTConfig &, vector<box> &) {
...
void contractor::prune_with_assert(box & b, SMTConfig & config, vector<box> & bin) {
