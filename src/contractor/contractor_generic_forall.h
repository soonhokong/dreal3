...
private:
    ...
    void handle(box & b, std::vector<box> & bin, Enode * body, bool const p, SMTConfig & config);
    std::vector<Enode *> elist_to_vector(Enode * e) const;
    void handle_disjunction(box & b, std::vector<box> & bin, std::vector<Enode *> const & vec, bool const p, SMTConfig & config);
    void handle_conjunction(box & b, std::vector<box> & bin, std::vector<Enode *> const & vec, bool const p, SMTConfig & config);
    void handle_atomic(box & b, std::vector<box> & bin, Enode * body, bool const p, SMTConfig & config);
