...
public:
    ...
    bool is_bisectable() const { return m_values.is_bisectable(); }
    ...
    // get_value
    ibex::Interval & get_value(int i) { return m_values[i]; }
    ibex::Interval const & get_value(int i) const { return m_values[i]; }
    ...
    // operator[]
    const ibex::Interval& operator[](int i) const { return get_value(i); }
